"""
A T5 based query expansion model is trained on topiocqa dataset.
(In the topiocqa dataset, the rewrite is generated by a T5 model trained on Qrecc.)
So for decode_type = 'answer', the rewrite of topiocqa is actually useless.
don't know why they integrade rewrite in the data structure?
"""
import argparse
import os
import json
...
dataset_dir = "/home/wangym/data1/dataset/topiocqa/"
collection_tsv = dataset_dir + "full_wiki_segments.tsv"
collection_json = dataset_dir + "full_wiki_segments.jsonl"
train = dataset_dir + "topiocqa_train.json"
dev = dataset_dir + "topiocqa_dev.json"
train_gold = dataset_dir + "ir_all_history_train.json"
dev_gold = dataset_dir + "ir_all_history_dev.json"
#dict_keys(['question', 'question_tokens', 'title', 'short_answers', 'conv_id', 'turn_id', 'doc_url', 'example_id', 'context'])
#question is in 'ALLHISTORY' format seperated by [SEP]
train_rewrite = dataset_dir + "ir_rewrite_train.json"
dev_rewrite = dataset_dir + "ir_rewrite_dev.json"
# same as ir_all_history_train.json, but with question in rewrite format
train_new = dataset_dir + "train_new.json"
dev_new = dataset_dir + "dev_new.json"
train_trec_gold = dataset_dir + "train_gold.trec"
dev_trec_gold = dataset_dir + "dev_gold.trec"
...
def convert_collection(collection_tsv, collection_json):
    ...
    with open(collection_tsv, 'r') as input, open(collection_json, 'w') as output:
        ...
        for i , row in enumerate(tqdm(reader)):
            ...
            #break
            obj = {"contents": " ".join([title, text]), "id": f"doc{id}"} # doc10
            output.write(json.dumps(obj, ensure_ascii=False) + '\n')

def load_collection(collection_file, title = True):
...
# combine original data and gold passage from ir_all_history and rewrites from ir_rewrite for training
def combine_data_train(inputs, inputs_gold, inputs_rewrite, output, collection):
    with open(inputs, "r") as f, open(inputs_gold, "r") as gf, open(inputs_rewrite, "r") as rw, open(output, "w") as g:
        ...
        total_nums = len(obj)
        # load collection for sampling negatives
        ...
        for i in range(total_nums):
            ...
            history_answer = []
            idx = 0 #check how many turns in a sample
...
# no need for sampling neagatives, and hence no need to load collection
